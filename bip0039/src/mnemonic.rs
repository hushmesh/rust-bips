...
impl<L: Language> Mnemonic<L> {
    ...
    /// ```
    pub fn from_entropy<S: Sha, E: Into<Vec<u8>>>(entropy: E) -> Result<Self, Error> {
        ...
        let checksum_byte = S::hash(&entropy)[0];
    ...
    /// ```
    pub fn from_phrase<'a, S: Sha, P: Into<Cow<'a, str>>>(phrase: P) -> Result<Self, Error> {
        let phrase = phrase.into();
        let entropy = Self::phrase_to_entropy::<S, _>(phrase.as_ref())?;
        Ok(Mnemonic {
            lang: PhantomData::<L>,
...
"##
    ...
    pub fn validate<'a, S: Sha, P: Into<Cow<'a, str>>>(phrase: P) -> Result<(), Error> {
        let _entropy = Self::phrase_to_entropy::<S, P>(phrase)?;
        Ok(())
    }

    fn phrase_to_entropy<'a, S: Sha, P: Into<Cow<'a, str>>>(phrase: P) -> Result<Vec<u8>, Error> {
